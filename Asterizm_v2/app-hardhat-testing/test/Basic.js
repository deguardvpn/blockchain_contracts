...
describe('DeGuard NFT', function () {
  ...
  it('Add new plan to the token', async function () {
    const range = 30;
    const receipt = await token1.connect(owner).addPlan(1, price, range);
    expect(receipt).to.emit(token1, 'PlanUpdated').withArgs(1, price, range);
  });

  it('Get plan list from token', async function () {
    const plans = await token1.connect(owner).getPlanList();
    plans.forEach((plan, index) => {
      const [id, price, range] = plan;
      console.log(
  ...
  it('Buy plan by user', async function () {
    ...
    const receipt = await token1.connect(client).buyPlan(planId, 1, client.address, { value });
    expect(receipt).to.emit(token1, 'PlanSold').withArgs(client, planId);
  });

  it('Plan balance', async function () {
    expect(await token1.balanceOf(client)).to.equal(1);
  });
