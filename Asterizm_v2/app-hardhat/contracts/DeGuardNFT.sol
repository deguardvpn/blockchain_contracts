...
contract DeGuardNFT is ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, IMultiChainToken, AsterizmClientUpgradeable {
    ...
    ///
    function safeMint(address to, uint _id, uint _startTime, uint _endTime) internal {
    ...
    function buyPlan(uint _plan, uint64 _dstChainId, address _to, bool _crosschain) public payable nonReentrant {
    ...
    function getPlanList() public view returns (Plan[] memory) {
        ...
        for(uint i = 0; i < plans.length; i++){
            _plans[i] = Plan(plans[i].id, plans[i].price, plans[i].range / _days);
        }
        return _plans;
    }
    ...
    function crossChainTransfer(uint64 _dstChainId, address _from, address _to, uint _tokenId) public payable {
        ...
        _initAsterizmTransferEvent(_dstChainId, abi.encode(_to, _tokenId, _getTxId(), plan.id, plan.startTime, plan.endTime));
    }

    /// Receive non-encoded payload
    /// @param _dto ClAsterizmReceiveRequestDto  Method DTO
    function _asterizmReceive(ClAsterizmReceiveRequestDto memory _dto) internal override {
        (address dstAddress, uint tokenId, , uint id, uint startTime, uint endTime) = abi.decode(_dto.payload, (address, uint, uint, uint, uint, uint));
    ...
    function _buildPackedPayload(bytes memory _payload) internal pure override returns(bytes memory) {
        (address dstAddress, uint tokenId, uint txId, uint id, uint startTime, uint endTime) = abi.decode(_payload, (address, uint, uint, uint, uint, uint));
        
        return abi.encodePacked(dstAddress, tokenId, txId, id, startTime, endTime);
    }
